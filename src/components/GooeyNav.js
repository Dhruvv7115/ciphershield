...
const GooeyNav = ({
  ...
  useEffect(() => {
    const newIdx = items.findIndex(item => item.href === pathname);
  ...
  const updateEffectPosition = element => {
    ...
    if (aEl) {
      if (aEl.children.length === 0 || aEl.innerText.trim() !== '') {
        textRef.current.innerHTML = aEl.innerHTML;
      } else {
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{
        ...
        `}} />
      <div className="relative" ref={containerRef} style={{ display: 'flex', alignItems: 'center' }}>
            ...
            {items.map((item, index) => (
              ...
              >
                <Link
                  ...
                  style={{ padding: '0.3em 0.7em', textDecoration: 'none', color: 'inherit', fontFamily: 'var(--font-sans)', fontSize: 14, fontWeight: 500, display: 'flex', alignItems: 'center' }}
                >
                  {item.label}
                </Link>
